...
| 菜肴（Dish） | id, name_fr, name_zh, era, origin, description, remySensory, keyIngredients[], chef, image | 每个时期 5–8 道，共 ~35 个节点 |
...
```json
{
  ...
  "chef": "相关厨师（可空）",
  "image": "/dishes/bouillabaisse.jpg"
}
```

`image` 字段为本地静态路径（`/dishes/[id].jpg`），指向 `public/dishes/` 目录下的对应图片。若无图片则为 `null`，面板自动降级显示（无占位符/破损图标）。
...
| 层次 | 内容 | 语气 |
|------|------|------|
| 默认展开 | 菜肴图片（全宽，192px 高）、菜肴法语/中文名、年代、起源、80–120字历史描述、关键食材 | 语气 B：信息密度为主 |
| 点击"听 Remy 说" | Remy 第一人称感官旁白（80–120字），展开动画 0.3s ease-out | 语气 C：感官为主，几乎无显性历史信息 |

菜肴图片展示于面板顶部，全宽出血至面板左右边界，底部以渐变过渡至面板背景色（`rgba(247,243,234,0.95)`），顶部附一条 3px 该时期色条。若 `image` 为 `null` 或图片加载失败，面板无缝降级（无破损图标）。
...
| Wikimedia Commons | 图片库 | 公共领域菜肴照片 | 直接下载，已本地化存储 |
...
26 道菜肴配有图片，均来自 Wikimedia Commons 公共领域（CC 授权），已下载至 `public/dishes/` 目录本地存储，不依赖外部网络。2 道菜肴（Oublie、Farcement）因 Wikimedia Commons 无准确对应图片，暂保持 `null`。
...
| 图片 | next/image + 本地静态资源 | 自动优化、懒加载、格式转换；图片存于 `public/dishes/` |
...
- `lib/types.ts` — `Dish` 接口含 `image?: string | null`；`SelectedEdge` 接口（sourceId / targetId / type / reason / x / y），`AppState` 含 `selectedEdge`
...
- `components/DishPanel` — 菜肴详情面板，入场动画 class `explore-panel-right`；包含：
  - `DishInfo` — 语气 B 史实信息，顶部展示菜肴图片（有图时全宽出血，含渐变遮罩和时期色条；无图时无缝降级）
...
MVP 阶段全部数据存为静态 JSON，图片存为静态文件：

```
/data
  eras.json           // 5 个时期节点
  dishes.json         // 28 道菜肴节点（含 remySensory、image 字段）
  edges.json          // evolved_from + era_sibling 两种边，每条边含 reason 字段
...
| Phase 1.6：视觉品质 | — | **已完成（2026-05-10）**：首页叙事入口（`/`）+ 探索路由（`/explore`）拆分；全站动画系统；色值统一；图谱交互细节 |
| Phase 1.7：图片系统 | — | **已完成（2026-05-11）**：首页布局间距修正；菜肴图片系统（`image` 字段 + DishInfo 图片展示 + 26 张本地图片） |
...
**首页布局间距修正（/impeccable layout homepage）**

- Era dots 行：顶部/底部 padding 从 22px/6px 不对称值改为 `clamp(20px,2.6vh,30px)` / `clamp(10px,1.6vh,18px)`，消除头部区域漂浮感
- 右侧叙事栏间距从单调的 18–42px 平坦区间重构为三档分明的节奏：
  - 副标题 → 分隔线：`clamp(6px,0.8vh,10px)`（紧凑，标签与装饰为一组）
  - 分隔线 → Remy 引言：`clamp(24px,3vh,38px)`（中等，开阔引言入口）
  - Remy 引言 → 元信息：`clamp(36px,4.8vh,60px)`（慷慨，引言是主角）
  - 元信息 → CTA：`clamp(16px,2vh,26px)`（紧凑，元信息与 CTA 为一组）
- 分隔线宽度从 28px 改为 36px（28px 在视觉上过于模糊，36px 读为明确的装饰标记）

**菜肴图片系统**

- `lib/types.ts`：`Dish` 接口新增 `image?: string | null` 可选字段
- `components/DishPanel/DishInfo.tsx`：面板顶部新增图片区域，全宽出血（`width: calc(100% + 3rem)`，`marginLeft: -1.5rem`），高 192px，底部渐变淡入面板背景色，顶部 3px 时期色条；图片加载失败时通过 `useState(imgError)` + `onError` 无缝降级，无破损占位符
- `data/dishes.json`：28 道菜肴全部新增 `image` 字段；26 道填入本地路径 `/dishes/[id].jpg`，2 道（Oublie、Farcement）保持 `null`
- `public/dishes/`：新建目录，存放 26 张从 Wikimedia Commons 下载的公共领域菜肴图片，总计约 10MB；图片命名与菜肴 `id` 一一对应
- `next.config.ts`：图片改用本地存储后，移除原有 `remotePatterns`（`upload.wikimedia.org`）配置
...
*文档结束 · La Table du Temps v1.5 · 如有更新请以最新版本为准*
